<template>
  <div class="p-6 max-w-7xl mx-auto font-sans bg-sand-50/50 min-h-screen rounded-3xl page-enter-anim">
    <PageHeader :title="pageTitle" subtitle="Découvrez l'ensemble des prestations proposées à nos clients.">
      <template #actions>
        <BaseButton v-if="activeCategory" variant="ghost" size="sm" @click="clearCategoryFilter">
    ...
    <EmptyState
      ...
      :title="activeCategory ? 'Aucun service dans cette catégorie' : 'Aucun service disponible'"
      :description="activeCategory ? 'Ajoutez un service dans cette catégorie ou revenez à la liste complète.' : 'Les services apparaîtront ici une fois ajoutés.'"
    />
      ...
      >
        ...
        <div class="absolute top-3 right-3 flex items-center gap-2">
          <button
            type="button"
            title="Modifier"
            class="w-8 h-8 rounded-lg bg-white/15 backdrop-blur-sm border border-white/20 flex items-center justify-center text-white hover:bg-white/25 transition-colors duration-200 cursor-pointer"
          ...
          <button
            type="button"
            title="Supprimer"
            class="w-8 h-8 rounded-lg bg-white/15 backdrop-blur-sm border border-white/20 flex items-center justify-center text-white hover:bg-coral-500/80 transition-colors duration-200 cursor-pointer"
    ...
    <Modal v-model="showPopup" size="lg">
      ...
      <form @submit.prevent="saveService" class="space-y-6">
        <div>
          ...
          <div class="space-y-4">
            <FormField
              ...
              :error="errors.titre"
            />
            <SelectField v-model="form.icon" label="Catégorie" :invalid="!!errors.icon" :error="errors.icon">
            ...
            />
            <ImageUpload v-model="form.image" label="Photo du service" folder="services" />
          </div>
        </div>
        ...
        <div>
          <h4 class="text-sm font-semibold text-navy-400 mb-3 pb-2 border-b border-sand-200">Caractéristiques</h4>
          <div class="flex gap-2 mb-3">
            ...
            />
            <BaseButton type="button" variant="secondary" @click="addCaracteristique">Ajouter</BaseButton>
          </div>
    ...
    <Modal :model-value="deleteId !== null" size="sm" @update:model-value="cancelDelete">
      <h3 class="text-lg font-bold text-navy-500 mb-2">Supprimer ce service ?</h3>
      ...
      <div class="flex justify-end gap-3">
        ...
        <BaseButton variant="danger" @click="deleteService">Supprimer</BaseButton>
      </div>
    </Modal>
  </div>
</template>
...
const activeCategory = computed(() => {
  ...
  return typeof value === 'string' && iconOptions.some((o) => o.value === value)
    ? (value as ServiceIconType)
    : null
})

const filteredServices = computed(() =>
  activeCategory.value ? services.value.filter((s) => s.icon === activeCategory.value) : services.value,
)

const pageTitle = computed(() => {
  if (!activeCategory.value) return "Services de l'hôtel"
  return iconOptions.find((o) => o.value === activeCategory.value)?.label ?? "Services de l'hôtel"
})
...
type ServiceFormData = Omit<Service, 'id' | 'slug' | 'docId'>
...
const form = ref<ServiceFormData>(emptyForm())
...
function uniqueSlug(titre: string, excludeId: number | null) {
  const base = slugify(titre) || 'service'
  let slug = base
  let counter = 2
  while (services.value.some((s) => s.slug === slug && s.id !== excludeId)) {
    slug = `${base}-${counter}`
    counter += 1
  }
  return slug
...
function addCaracteristique() {
  ...
  if (value && !form.value.caracteristiques.includes(value)) {
...
function validate(): boolean {
  ...
  if (!form.value.icon) errors.value.icon = 'La catégorie est requise.'
  if (!form.value.description.trim()) errors.value.description = 'La description est requise.'
  if (!form.value.horaires.trim()) errors.value.horaires = 'Les horaires sont requis.'
  if (!form.value.info.trim()) errors.value.info = 'Le tarif ou la capacité est requis.'
...
const openAddPopup = () => {
  ...
  editId.value = null
  form.value = { ...emptyForm(), icon: activeCategory.value ?? ('' as ServiceIconType) }
  errors.value = {}
...
const saveService = () => {
  if (!validate()) return
  const slug = uniqueSlug(form.value.titre, editId.value)
  if (isEditing.value && editId.value !== null) {
    servicesStore.update(editId.value, { ...form.value, slug })
  } else {
    servicesStore.add({ ...form.value, slug })
  }
  closePopup()
}
...
const deleteService = () => {
  if (deleteId.value !== null) {
    servicesStore.remove(deleteId.value)
    deleteId.value = null
  }
}

<template>
  <header class="flex justify-between items-center gap-4 bg-gray-800 px-6 py-4 shadow-md">
    <div class="shrink-0">
      <div class="text-2xl font-bold text-white tracking-tight">
        {{ currentTitle }}
      </div>
      <p class="text-xs text-gray-300 mt-0.5 capitalize">{{ todayLabel }}</p>
    </div>
    ...
    <div ref="searchContainer" class="relative hidden md:block flex-1 max-w-md">
      <div class="relative">
        ...
        <input
          ...
          class="w-full pl-9 pr-4 py-2 bg-white/10 border border-white/10 rounded-xl text-sm text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-200 focus:bg-white/15 transition"
      ...
      <Transition name="fade">
        <div
          ...
          class="absolute left-0 right-0 mt-2 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden z-50"
        >
          <button
            v-for="result in searchResults"
            :key="`${result.type}-${result.id}`"
            type="button"
            class="w-full text-left px-4 py-3 text-sm hover:bg-gray-50 transition cursor-pointer flex items-center gap-3 border-b border-gray-50 last:border-0"
            @click="goToResult(result)"
          >
            <Icon :name="result.type === 'client' ? 'clients' : 'booking'" class="w-4 h-4 text-gray-400 shrink-0" />
            <div class="min-w-0">
              <p class="text-gray-900 font-medium truncate">{{ result.label }}</p>
              <p class="text-xs text-gray-500">{{ result.type === 'client' ? 'Client' : 'Réservation' }}</p>
            </div>
          </button>
        </div>
      </Transition>
      ...
      <Transition name="fade">
        <div
          v-if="showResults && searchQuery.trim() && searchResults.length === 0"
          class="absolute left-0 right-0 mt-2 bg-white rounded-xl shadow-lg border border-gray-100 px-4 py-3 text-sm text-gray-500 z-50"
        >
    ...
    <div class="flex items-center gap-3 shrink-0">
      <button
        ...
        type="button"
        class="bg-white/10 hover:bg-white/15 text-white rounded-xl px-3 py-1.5 text-xs font-medium flex items-center gap-1.5 transition cursor-pointer"
